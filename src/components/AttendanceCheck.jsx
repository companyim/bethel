...
function AttendanceCheck() {
  ...
  return (
    <section className="section attendance-check">
      ...
      {checkMode === 'grade' && !selectedGrade && (
        <div className="empty-message">학년을 선택해주세요.</div>
      )}

      {checkMode === 'department' && !selectedDepartment && (
        <div className="empty-message">부서를 선택해주세요.</div>
      )}

      {((checkMode === 'grade' && selectedGrade) || (checkMode === 'department' && selectedDepartment)) && filteredStudents.length === 0 && (
        <div className="empty-message">해당 조건에 등록된 학생이 없습니다.</div>
      )}
        ...
        <>
          <div className="attendance-list">
            {filteredStudents.map((student, idx) => {
              ...
              return (
                <div key={`${student.name}-${student.grade}-${idx}`} className="attendance-item">
